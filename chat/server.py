    ...
    async with aiohttp.ClientSession() as session:
        async with session.get('https://api.privatbank.ua/p24api/pubinfo?json&exchange&coursid=5') as resp:
            if resp.status == 200:
                ...
                exc1, = list(filter(lambda el: el["ccy"] == "EUR", r))
            return f"USD: buy: {exc['buy']}, sale: {exc['sale']} EUR: buy: {exc1['buy']}, sale: {exc1['sale']}"


async def get_exc_usd():
    async with aiohttp.ClientSession() as session:
        async with session.get('https://api.privatbank.ua/p24api/pubinfo?json&exchange&coursid=5') as resp:
            if resp.status == 200:
                r = await resp.json()
...
async def get_exc_eur():
    async with aiohttp.ClientSession() as session:
        async with session.get('https://api.privatbank.ua/p24api/pubinfo?json&exchange&coursid=5') as resp:
            if resp.status == 200:
                r = await resp.json()
                exc, = list(filter(lambda el: el["ccy"] == "EUR", r))
            return f"EUR: buy: {exc['buy']}, sale: {exc['sale']}"
